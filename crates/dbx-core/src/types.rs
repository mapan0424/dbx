...
mod tests {
    ...
    fn list_objects_payload_preserves_optional_validity() {
        let objects: Vec<ObjectInfo> =
            serde_json::from_str(
                r#"[{"name":"TRG_AUDIT","object_type":"TRIGGER","schema":"APP","valid":false,"trigger":{"name":"TRG_AUDIT","event":"INSERT","timing":"BEFORE","level":"FOR EACH ROW","enabled":false,"valid":false,"comment":"audit","created_at":"2026-08-10 09:30:00"}}]"#,
            )
                .unwrap();
