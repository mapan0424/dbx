 ...
 * - VIEW/MATERIALIZED_VIEW/PROCEDURE/FUNCTION/TRIGGER/SEQUENCE/SYNONYM/PACKAGE/PACKAGE_BODY/TYPE/TYPE_BODY → open-source
 * - otherwise → none
 */
...
function canOpenSource(row: ObjectBrowserRow): boolean {
  return row.type === "VIEW" || row.type === "MATERIALIZED_VIEW" || row.type === "PROCEDURE" || row.type === "FUNCTION" || row.type === "TRIGGER" || row.type === "SEQUENCE" || row.type === "SYNONYM" || row.type === "PACKAGE" || row.type === "PACKAGE_BODY" || row.type === "TYPE" || row.type === "TYPE_BODY";
}
